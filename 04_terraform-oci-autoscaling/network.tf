...
resource "oci_core_subnet" "CCCWebSubnet" {
  ...
  security_list_ids = [oci_core_security_list.CCCWebSecurityList.id,oci_core_security_list.CCCSSHSecurityList.id]
}
...
resource "oci_core_subnet" "CCCLBSubnet" {
  ...
  dhcp_options_id = oci_core_dhcp_options.CCCDhcpOptions1.id
  security_list_ids = [oci_core_security_list.CCCWebSecurityList.id]
}
...
resource "oci_core_subnet" "CCCBastionSubnet" {
  ...
  route_table_id = oci_core_route_table.CCCRouteTableViaIGW.id
  dhcp_options_id = oci_core_dhcp_options.CCCDhcpOptions1.id
  security_list_ids = [oci_core_security_list.CCCSSHSecurityList.id]
}
